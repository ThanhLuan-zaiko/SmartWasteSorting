...
function appendLogLine(
  ...
  return {
    ...base,
    stdout:
      stream === "stdout" ? trimLogTail(base.stdout, line) : base.stdout,
    stderr:
      stream === "stderr" ? trimLogTail(base.stderr, line) : base.stderr,
  };
}
...
export function LocalAgentProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleJobStreamEvent = useEffectEvent(async (event: JobStreamEvent) => {
    ...
    if (event.event === "resync_required") {
      try {
        ...
        setConnectionError(
          error instanceof Error ? error.message : event.reason,
        );
      }
    }
  });
  ...
  useEffect(() => {
    setActiveLogs((current) =>
      current && current.job_id === activeJobId ? current : null,
    );

    const socket = new WebSocket(
      buildJobsWebSocketUrl(activeJobId, LIVE_LOG_TAIL_LINES),
    );
  ...
  const value: LocalAgentContextValue = {
    runs,
    jobs,
    streamConnected,
    selectedExperiment,
