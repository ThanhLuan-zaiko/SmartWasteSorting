...
fn warm_cache_requires_existing_manifest() {
    ...
    let missing_manifest = std::env::temp_dir().join(format!(
        "localagent_missing_manifest_{}_warm_cache.parquet",
        std::process::id()
    ));
...
fn evaluate_requires_existing_checkpoint() {
    ...
    let manifest_path = temp_root.join("dataset_manifest.parquet");
...
fn fit_requires_at_least_two_labels_in_dataset_summary() {
    ...
    std::fs::write(
        ...
        r#"{"training_ready_files":9989,"trainable_label_counts":{"r":9999}}"#,
    )
